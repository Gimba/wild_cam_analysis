...
Using these [pictures](#difference) for training an image classification neural net did not result in the desired accuracy and it got apparent, that object recognition is maybe better suited for the task. The next thing I tried was to use AWS Rekognition on [extracted frames](#extracted). The bird was detected with a confidence of 94%). There should be a box around it, but somehow the box coordinates are outside of the image. I could not figure out why at this point.
